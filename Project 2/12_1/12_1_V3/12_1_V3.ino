...
#define kp_front 0.85
#define ki_front 0.000
#define kd_front 0.02
...
void PID() {
  ...
  Fvalue = map(Fvalue, -180, 180, 135, 60);
  ...
  setVehicleHR(Fvalue, throttle);

//  throttle = max(min(throttle, 95), 83);
//  diff = throttle - 90;
//  if (abs(diff) < 5) {
//    if (diff < 0){
//      setVehicleHR(Fvalue, 83);
//      delay(int(abs(diff) * 15));
//    }
//    else if (diff > 0)
